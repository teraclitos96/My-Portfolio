...
describe('book list keyboard navigation', () => {
  beforeEach(() => {
    document.body.innerHTML = `
      <ul data-book-navigation-page="2">
  ...
  test('focuses and cycles forward through list items', () => {
    ...
    expect(moveBookListFocus({ currentPage: 2, direction: 1 })).toBe(true)
    expect(document.activeElement).toBe(items[0])

    moveBookListFocus({ currentPage: 2, direction: 1 })
    moveBookListFocus({ currentPage: 2, direction: 1 })
    moveBookListFocus({ currentPage: 2, direction: 1 })
    expect(document.activeElement).toBe(items[0])
  })
  ...
  test('focuses the first item when entering a navigation page', () => {
    ...
    expect(focusFirstBookListItem({ currentPage: 2 })).toBe(true)
  ...
  test('does nothing outside a navigation page', () => {
    expect(moveBookListFocus({ currentPage: 5, direction: 1 })).toBe(false)
  })
  ...
  test('enters, moves through and leaves page navigation', () => {
    document.body.insertAdjacentHTML('beforeend', `
      <nav data-book-page-navigation="4">
        <button>Index</button>
        <button>All projects</button>
      </nav>
    `)
    const items = document.querySelectorAll('[data-book-page-navigation] button')

    expect(focusFirstBookPageNavigationItem({ currentPage: 5 })).toBe(true)
    expect(document.activeElement).toBe(items[0])

    moveBookPageNavigationFocus({ currentPage: 5, direction: 1 })
    expect(document.activeElement).toBe(items[1])
